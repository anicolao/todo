...
Put current shared/pending people in a **Shared with** section and available people in a separate **Add people** section, each with its own heading and white card. Render each recipient’s stored profile photo in a circular avatar. Keep rows stable while toggling, and expose full-row targets with semantic checkboxes. Search filters both groups and announces the result count. Use “No matching people” for a search miss and “No other people available yet” for an empty directory; keep Back available and disable Save until there is a valid change. Fall back to email when a name is missing and initials only when a photo is missing or fails to load. If identity cannot be resolved, show an unavailable row and explain why it cannot be selected.
